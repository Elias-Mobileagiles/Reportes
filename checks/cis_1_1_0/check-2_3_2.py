...
class Check_CIS_2_3_2(Checker):
    ...
    def do_check(self):
        ...
        if config_system_snmp_community and 'edits' in config_system_snmp_community:
            if len(config_system_snmp_community['edits']) > 0:
                ...
                self.add_message('There are no SNMP communities defined')

        # Check the SNMPv3 users configured
        config_system_snmp_user = self.get_config("system snmp user")

        if config_system_snmp_user and 'edits' in config_system_snmp_user:
            ...
            self.set_message('No SNMP user configuration found')
